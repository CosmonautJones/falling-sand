...
## Canvas-first phone drawer: September 17

This supersedes the preceding phone layout. Phone controls now live in a modal bottom drawer above a compact material/Pause/More bar. Portrait fills the play area with a cropped view of the unchanged 480 × 270 world; Whole vessel fits it. Materials and brush sizes share a drawer, with labeled swatches and brush previews. Selecting a material closes it. More holds camera controls, speed, Codex, the first experiment and clear/reset. Clear/reset ask before replacing the world. Codex is a native modal with keyboard focus containment.

- TDD failures were observed for portrait camera geometry, two-finger anchoring, the absent material drawer, painting continuing when a second finger joined, and keyboard focus escaping Codex. The corresponding checks passed after implementation.
- `scripts/mobile-qa.cjs` passes at 390 × 844, 375 × 667, 844 × 390 and 667 × 375. Closed play heights are 733, 556, 279 and 264 pixels respectively, with no document overflow or stretched grains. Drawer targets measure at least 44 × 44 pixels. Opening controls preserves camera and cells; rotation preserves cells; material selection closes the drawer; canceling clear preserves the world; Escape restores drawer-opener focus.
- Chromium CDP touch dispatch verifies single-finger pouring, two-finger pan/zoom, no pouring after one finger lifts, fresh pouring after all fingers lift, cancellation, and the one-finger Move view alternative. Resizing to desktop with a drawer open leaves one set of usable controls and no orphaned modal.
- Desktop Chromium verifies right-click naming, right-drag pan, double-click fit, and independent pause/brush state. Desktop WebKit 18.5 emulation verifies both orientations, drawer/material selection, the first experiment, Move view, Codex, clear cancellation and whole-vessel fit, with no page errors. Screenshots were visually inspected. This does not certify physical iPhone Safari, safe-area delivery, thermal behavior or native tab suspension.
- All 201 tests in 14 files pass (77.81 seconds, one worker); lint, TypeScript and the production build pass. The existing bundle warning remains: 563.11 kB JavaScript, 147.92 kB gzip. No new dependency, simulation rule or seed-vessel change is included.
- A fresh 30-second hands-off phone opening advanced from beat 8 to 1809, retaining four mites, seven mud grains and mud at both nursery cells `(131,264)` and `(132,264)`. The HUD sampled 60 fps, with no page errors. Screenshot: `qa-shots/drawer-opening-30s.png`. This is one desktop-emulated observation, not a sustained phone-performance guarantee.

Next simulation slice: prototype a boiler and narrow vent using heat, trapped steam, directional release and condensation. Pressure transport and conservation need evidence before wall rupture or additional effects. This phone-interface release does not implement pressure or change gas behavior.
